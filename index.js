...
const TAG = '<all-img-base64>';
const END = '</all-img-base64>';
const EXTIMG = /(\<img)(\s)+/gi;
const EXTQUOT = /^(\s)*\=?(\s)*[\"\'].*/g;
const LIMIT = 8 * 1024;
const PLUGIN_NAME = 'gulp-allimgbase64';
const lujing = [];

function plugin(option) {
  let limit = option ? fileSize(option.limit) : LIMIT;
  let dbfile = option.deleteAfterEncoding;
    ...
    if (file.isBuffer()) {
      ...
      var processPiece = function (i) {
        ...
        for(;;){
          ...
          Block[i].img_64.push(transBase64(path_regular(piece.substring(i_start + 1, i_end), limit, html_path)));			
...
function washTags(html){
  html = html.replace(/\<all-img-base64\>/g, '');
  html = html.replace(/\<\/all-img-base64\>/g, '');
  return html;
}
...
function delfile(){
	var x;
	for (x in lujing){
		fs.unlink(lujing[x], function(err) {
         if (err) {
           return ;
          }			
		});
